...
static int init(AVFilterContext *ctx)
{
    ...
    if (wctx->vad_model_path) {
        ...
        ctx_params.n_threads = wctx->threads;
        ...
        wctx->vad_params.max_speech_duration_s = wctx->audio_buffer_queue_size / 1000.0f;
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    if (wctx->ctx_vad
        ...
        if (!segments) {
            ...
            if (n_segments > 0) {
                const int64_t start_ms = whisper_vad_segments_get_segment_t0(segments,
                                                                             n_segments - 1) * 10;
                const int64_t end_ms = whisper_vad_segments_get_segment_t1(segments,
                                                                           n_segments - 1) * 10;
...
static const AVOption whisper_options[] = {
    ...
    { "language", "Language for transcription ('auto' for auto-detect)", OFFSET(language), AV_OPT_TYPE_STRING,
    ...
    { "threads", "Number of threads to use", OFFSET(threads), AV_OPT_TYPE_INT, {.i64 = 4}, 0, INT_MAX,.flags = FLAGS },
    ...
    { "vad_threshold", "VAD threshold", OFFSET(vad_threshold), AV_OPT_TYPE_FLOAT, {.dbl = 0.5}, 0.0, 1.0,.flags =
     FLAGS },
    { "vad_min_speech_duration", "Minimum speech duration in milliseconds for VAD", OFFSET(vad_min_speech_duration),
     AV_OPT_TYPE_INT, {.i64 = 50}, 20, INT_MAX,.flags = FLAGS },
    { "vad_min_silence_duration", "Minimum silence duration in milliseconds for VAD", OFFSET(vad_min_silence_duration),
     AV_OPT_TYPE_INT, {.i64 = 500}, 0, INT_MAX,.flags = FLAGS },
    { NULL }
};
